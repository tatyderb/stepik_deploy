...
class SnapshotManager:
    ...
    def create_snapshot(self, lesson: Lesson, md_filename: str) -> str:
    ...
    def _extract_step_text_preview(self, step_dict: Dict) -> str:
        """Извлекает превью текста шага для отладки"""
        try:
            text = step_dict.get('stepSource', {}).get('block', {}).get('text', '')
            if text:
                # Берем первую строку и обрезаем
    ...
    def verify_lesson(self, lesson: Lesson, md_filename: str) -> Dict[str, Any]:
        ...
        print(f"Проверка снапшота ({len(snapshot['steps'])} шагов) vs текущего урока ({len(current_steps)} шагов)")
        ...
        for position in range(1, max_steps + 1):
            ...
            step_result = self._compare_steps(position, snapshot_step, current_step)
    ...
    def _compare_steps(self, position: int, snapshot_step: Optional[Dict], current_step: Optional[Step]) -> Dict[str, Any]:
    ...
    def verify_single_step(self, lesson: Lesson, md_filename: str, step_position: int) -> Dict[str, Any]:
    ...
    def _compare_dicts(self, dict1: Dict, dict2: Dict, path: str = "") -> List[str]:
        ...
        for key in all_keys:
            ...
            if key not in dict1:
                differences.append(f"➕ Добавлено поле: {current_path} = {self._truncate_value(dict2[key])}")
            elif key not in dict2:
                differences.append(f"➖ Удалено поле: {current_path} = {self._truncate_value(dict1[key])}")
            elif isinstance(dict1[key], dict) and isinstance(dict2[key], dict):
                # Рекурсивное сравнение вложенных словарей
                nested_diffs = self._compare_dicts(dict1[key], dict2[key], current_path)
                ...
                if current_path.endswith("test_cases"):
                    ...
                    if normalized1 != normalized2:
                        differences.append(f"Изменено поле: {current_path}\n   Было: {self._truncate_value(dict1[key])}\n   Стало: {self._truncate_value(dict2[key])}")
                elif dict1[key] != dict2[key]:
                    # Для текстовых полей показываем более информативное сравнение
                    if key == "text" or "text" in current_path:
                        ...
                        if len(old_lines) > 3 or len(new_lines) > 3:
                            differences.append(f"Изменен текст: {current_path}\nБыло ({len(old_lines)} строк):\n      " + "\n      ".join(old_lines[:3]) + ("\n      ..." if len(old_lines) > 3 else ""))
                            differences.append(f"   Стало ({len(new_lines)} строк):\n      " + "\n      ".join(new_lines[:3]) + ("\n      ..." if len(new_lines) > 3 else ""))
                        else:
                            differences.append(f"Изменено поле: {current_path}\n   Было: {self._truncate_value(dict1[key])}\n   Стало: {self._truncate_value(dict2[key])}")
                    else:
                        differences.append(f"Изменено поле: {current_path}\n   Было: {self._truncate_value(dict1[key])}\n   Стало: {self._truncate_value(dict2[key])}")
...
@click.option('-s', '--step', type=int, default=0, metavar='STEP', 
              help='Сверить только конкретный шаг (нумерация с 1, отрицательные - с конца)')
...
def main(filename: str, step: int, update: bool):
    ...
    try:
        ...
        if update:
            ...
            result = manager.create_snapshot(lesson, filename)
            click.echo(result)
        
        elif step != 0:
            # Проверяем один шаг
            result = manager.verify_single_step(lesson, filename, step)
